...
export default function ItemDetails({ route }: ItemDetailsProps) {
  ...
  const {
    ...
    error,
    refetch,
  } = useGet<OutfitOverview[]>(getOutfitsAsync(db));
  ...
  return (
    <SafeAreaView>
      <ScrollView
        ...
        onScroll={(e) => headerOnScrollTransition(e, navigation, headerHeight)}
        contentInsetAdjustmentBehavior="never"
        // onScroll={(event) =>
        //   route.params.setScrollPosition(event.nativeEvent.contentOffset.y)
        // }
      >
        <View style={styles.image}>
          {item.image ? (
            ...
            <Image
              source={require("../styles/img/noImg.jpg")}
              style={{ resizeMode: "cover", width: "100%", height: "100%" }}
            />
          )}
        </View>
        <View style={styles.content}>
          ...
          <View style={styles.details}>
            ...
            <Detail
              label="Cost per wear"
              value={
                item.cost && item.wears > 0
                  ? calculateCostPerWear(item.cost, item.wears)
                  : item.cost
              }
              suffix="€"
            />
          ...
          <View style={{ alignItems: "center", gap: 8 }}>
            {savedOutfits
              ?.filter((outfit) =>
                outfit.itemImageURLs?.some((i) => item.uuid === i.uuid)
              )
              .map((o) => (
                <PlannedOutfit
                  key={o.uuid}
                  label={o.name}
                  outfitImage={o.imageURL}
                  itemImages={o.itemImageURLs}
                />
              ))}
          </View>
